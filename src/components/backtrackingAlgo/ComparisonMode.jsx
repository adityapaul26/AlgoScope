...
function generateFrames(n) {
  ...
  function snap(type, row, col, message) {
    frames.push({ board: board.map((r) => [...r]), type, activeRow: row, activeCol: col, message, solutions, backtracks })
  }

  function solve(row) {
    if (row === n) { solutions++; snap('solution', -1, -1, `✓ Solution #${solutions}`); return }
    for (let col = 0; col < n; col++) {
      ...
      board[row][col] = ''
      backtracks++
      snap('backtrack', row, col, `Row ${row + 1}: backtrack from col ${col + 1}`)
    }
  }
  solve(0)
  return frames
...
function MiniBoard({ n, frames, speed, trigger, accent }) {
  ...
  const cellStyle = (r, c) => {
    ...
    if (isActive && frame?.type === 'backtrack') return 'bg-orange-500/50 border-orange-400'
    if (hasQueen && frame?.type === 'solution')  return 'bg-emerald-500 border-emerald-300'
  ...
  return (
    <div className="flex-1 rounded-xl border bg-slate-900/60 overflow-hidden" style={{ borderColor: accent + '44' }}>
      {/* Panel header */}
      <div className="px-5 py-3 flex items-center justify-between border-b" style={{ borderColor: accent + '33', background: accent + '11' }}>
        <span className="text-sm font-bold" style={{ color: accent }}>{n}×{n} Board</span>
        {done && (
      ...
      <div className="p-5 flex flex-col items-center gap-5">
        {/* Board */}
        <div className="grid gap-0.5" style={{ gridTemplateColumns: `repeat(${n}, ${cellPx}px)` }}>
          {board.map((row, r) =>
            row.map((cell, c) => (
              <div
                key={`${r}-${c}`}
        ...
        <div className="grid grid-cols-2 gap-3 w-full">
          <div className="rounded-lg bg-slate-800/60 p-3 border border-slate-700 text-center">
            ...
            <h3 className="text-xl font-bold mt-0.5" style={{ color: accent }}>{solutions}</h3>
          </div>
          <div className="rounded-lg bg-slate-800/60 p-3 border border-slate-700 text-center">
            <p className="text-slate-400 text-xs">Backtracks</p>
            <h3 className="text-xl font-bold text-orange-400 mt-0.5">{backtracks}</h3>
          </div>
        </div>

        <StatusDisplay message={frame?.message ?? `${n}×${n} N-Queens — click Run Both to start`} />
      </div>
    </div>
  )
}
  ...
  return (
    <div className="p-4 sm:p-6 flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-center gap-4 p-4 rounded-xl bg-slate-900/50 border border-white/5">
        <div>
          <label className="text-xs text-slate-400 mb-1 block">Board size pair</label>
          <div className="flex rounded-xl overflow-hidden border border-slate-700">
            {SIZE_PAIRS.map((p, i) => (
              <button
                key={i}
                onClick={() => { setPair(i); setTrigger(0) }}
                className={`px-4 py-2 text-xs font-bold transition-all duration-200 ${
                  pair === i ? 'bg-cyan-500 text-black' : 'bg-slate-800 text-slate-400 hover:text-white'
                }`}
              >
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        <MiniBoard n={left}  frames={leftFrames}  speed={speed} trigger={trigger} accent="#06b6d4" />
        <MiniBoard n={right} frames={rightFrames} speed={speed} trigger={trigger} accent="#a855f7" />
      </div>

      {/* Step count insight */}
      {trigger > 0 && (
        <div className="rounded-xl bg-slate-900/50 border border-white/5 p-5">
          <p className="text-xs text-slate-500 uppercase tracking-widest mb-3">Complexity Insight</p>
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1 rounded-xl bg-slate-800/60 p-4 border border-cyan-500/20 text-center">
              <p className="text-slate-400 text-xs">{left}×{left} total frames</p>
              <h2 className="text-3xl font-bold text-cyan-400 mt-1">{leftFrames.length}</h2>
            </div>
            <div className="flex-1 rounded-xl bg-slate-800/60 p-4 border border-purple-500/20 text-center">
              <p className="text-slate-400 text-xs">{right}×{right} total frames</p>
              <h2 className="text-3xl font-bold text-purple-400 mt-1">{rightFrames.length}</h2>
            </div>
            <div className="flex-1 rounded-xl bg-emerald-500/10 p-4 border border-emerald-500/20 text-center">
              ...
              <h2 className="text-3xl font-bold text-emerald-400 mt-1">
                {(rightFrames.length / Math.max(leftFrames.length, 1)).toFixed(1)}×
              </h2>
              <p className="text-slate-500 text-xs mt-1">more work for +{right - left} rows</p>
            </div>
          </div>
        </div>
      )}
